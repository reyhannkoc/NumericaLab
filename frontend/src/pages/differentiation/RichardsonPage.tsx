...
const FINAL_FRAME = RICH_FRAMES[RICH_FRAMES.length - 1]
...
const RICH_TABLE: TableRow[] = FINAL_FRAME.hValues.map((hv, i) => ({
  level: i,
  h: hv.toExponential(4),
  central: (FINAL_FRAME.table[i]?.[0] ?? 0).toFixed(10),
  richardson: (FINAL_FRAME.table[i]?.[i] ?? 0).toFixed(10),
  error: Math.abs((FINAL_FRAME.table[i]?.[i] ?? 0) - DEMO_EXACT).toExponential(4),
}))
...
const CENTRAL_ERRORS = H_RANGE.map(hv => Math.abs(centralDiff(hv) - DEMO_EXACT))
const RICH_ERRORS = H_RANGE.map(hv => {
...
const VIZ_DATA: Plotly.Data[] = [
  {
    x: H_RANGE, y: CENTRAL_ERRORS.map(e => Math.max(e, 1e-17)),
    type: 'scatter', mode: 'lines', name: 'Central O(h²)',
    line: { color: '#8b5cf6', width: 2.5 },
  },
  {
    x: H_RANGE, y: RICH_ERRORS.map(e => Math.max(e, 1e-17)),
...
export default function RichardsonPage() {
  ...
  return (
    <LessonPage
      ...
      renderAlgorithm={() => (
        <AlgorithmExecution
          iterations={RICH_TABLE}
          columns={RICH_COLS}
          status="converged"
          description="Richardson extrapolation table (exact derivative in column R[0][k]) for the demonstration function f(x) = sin(x), x = 1."
        />
      )}
    />
  )
